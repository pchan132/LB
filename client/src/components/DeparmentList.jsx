...
export default function DeparmentList ({ userData }) {
  const departments = [
    "แผนกวิชาช่างยนต์",
  ...
  const renderDepartmentList = () => {
    if (!userData || !userData.length) return null; // ตรวจสอบว่ามีข้อมูลหรือไม่
      ...
      <>
        {[...departmentMap.keys()].map((department_id) => (
          ...
          >
            ...
            <div className="collapse-title text-lx font-semibold">
              {department_id}
            </div>
            <div className="collapse-content flex flex-col gap-2 ">
              {departmentMap.get(department_id).map((user, index) => (
                <div
                  key={index}
                  className="p-2  hover:bg-amber-200 rounded cursor-pointer"
                >
                  {user.receiver_name}
                </div>
              ))}
